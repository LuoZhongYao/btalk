...
#ifndef SBC_DEFAULT_BITPOOL
#define SBC_DEFAULT_BITPOOL 53
#endif
		...
		for (ch = 0; ch < frame->channels; ch++) {
			for (sb = 0; sb < frame->subbands; sb++) {
				...
				frame->sb_sample[blk][ch][sb] = (int32_t)
					((float)((((uint64_t) audio_sample << 1) | 1) << shift) /
...
struct sbc_priv {
	bool init;
	bool msbc;
	struct sbc_frame frame SBC_ALIGNED;
	struct sbc_decoder_state dec_state SBC_ALIGNED;
	struct sbc_encoder_state enc_state SBC_ALIGNED;
...
static void sbc_set_defaults(sbc_t *sbc, unsigned long flags)
{
	...
	sbc->bitpool = SBC_DEFAULT_BITPOOL;
#if __BYTE_ORDER == __LITTLE_ENDIAN
	sbc->endian = SBC_LE;
#elif __BYTE_ORDER == __BIG_ENDIAN
	sbc->endian = SBC_BE;
