...
/**
 * @brief       Enable GPIO pull state selection.
 * @param[in]   port          GPIO port. It could be PA, PB, PC or PD.
 * @param[in]   u32PinMask    The single or multiple pins of specified GPIO port.
 *                            It could be BIT0 ~ BIT15 for PA, PC and PD.
 *                            It could be BIT0 ~ BIT9, BIT12, BIT13, BIT14 for PB.
 * @param[in]   u32PullState  Pull state.  It could be \n
 *                            GPIO_PUSEL_PULL_UP, GPIO_PUSEL_PULL_DOWN.
 * @return      None
 * @details     This function is used to enable GPIO state(pull-up or pull-down).
 */
void GPIO_EnablePullState(GPIO_T *port, uint32_t u32PinMask, uint32_t u32PullState)
{
    ...
    for(i = 0; i < GPIO_PIN_MAX; i++)
    {
        if(u32PinMask & (1 << i))
        {
            port->PUSEL = (port->PUSEL & ~(0x3 << (i << 1))) | (u32PullState << (i << 1));
        }
    }
}

/**
 * @brief       Disable GPIO pull state selection.
 * @param[in]   port          GPIO port. It could be PA, PB, PC or PD.
 * @param[in]   u32PinMask    The single or multiple pins of specified GPIO port.
 *                            It could be BIT0 ~ BIT15 for PA, PC and PD.
 *                            It could be BIT0 ~ BIT9, BIT12, BIT13, BIT14 for PB.
 * @return      None
 * @details     This function is used to disable GPIO state(pull-up or pull-down).
 */
