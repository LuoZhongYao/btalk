...
void TIMER0_Init()
{
	...
	/* Set timer0 periodic time-out period is 10kHz if timer clock is 12 MHz */
	TIMER_Open(TIMER0, TIMER_PERIODIC_MODE, 100000);

	/* Enable timer interrupt trigger EADC */
	TIMER0->TRGCTL |= TMR_TRGCTL_TRGADC_Msk;
}
    ...
    while(1) {
        ...
        if(u8Option == '1') {
            ...
            while(1) {
                ...
                u32Temp = EADC_GET_CONV_DATA(EADC, 0);
                i32ConversionData[g_u32COVNUMFlag - 1] = u32Temp;
            ...
            {
              u32Temp = g_u32COVNUMFlag;
              printf("                                0x%X (%d)\n", i32ConversionData[u32Temp], i32ConversionData[u32Temp]);
            }
