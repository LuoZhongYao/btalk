...
/** @addtogroup I94100_USBD_Driver USBD Driver
  @{
*/


/** @addtogroup I94100_USBD_EXPORTED_FUNCTIONS USBD Exported Functions
  @{
*/
...
volatile uint8_t g_usbd_Configured = 0; 
...
static volatile uint8_t  g_usbd_CtrlInSizeZeroFlag = 0;
...
void USBD_Open(const S_USBD_INFO_T *param, CLASS_REQ pfnClassReq, SET_INTERFACE_REQ pfnSetInterface)
{
    ...
    /* Select USBD */
	SYS->USBPHY = (SYS->USBPHY) | SYS_USBPHY_USBPHYEN_Msk;
...
void USBD_GetDescriptor(void)
{
    ...
    switch(g_usbd_SetupPacket[3])
    {
		...
		case DESC_QUALIFIER:
		{
			u32Len = USBD_Minimum(u32Len, LEN_QUALIFIER);
			DBG_PRINTF("Get qualifier desc, %d\n", u32Len);

			USBD_PrepareCtrlIn((uint8_t *)g_usbd_sInfo->gu8QualiferDesc, u32Len);
		}
		break;
        // Get HID Descriptor
        case DESC_HID:
    ...
    if(g_usbd_SetupPacket[0] & 0x80)    /* request data transfer direction */
    {
        ...
        switch(g_usbd_SetupPacket[1])
        {
            ...
            case GET_INTERFACE:
            {
                ...
                /* Data stage */
              u32Temp = g_usbd_UsbAltInterface;
                M8(USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR(EP0)) = u32Temp;
                USBD_SET_DATA1(EP0);
                USBD_SET_PAYLOAD_LEN(EP0, 1);
                /* Status stage */
                ...
                if(g_usbd_SetupPacket[2] == FEATURE_ENDPOINT_HALT)
                {
                    ...
                    for(i = 0; i < USBD_MAX_EP; i++)
                    {
                        if(((USBD->EP[i].CFG & 0xF) == epNum) && ((g_u32EpStallLock & (1 << i)) == 0))
                        {
							...
							USBD_SET_DATA0(epNum);		// Reset Data Toggle
...
void USBD_PrepareCtrlIn(uint8_t *pu8Buf, uint32_t u32Size)
{
    ...
    if(u32Size > g_usbd_CtrlMaxPktSize)
    {
      uint32_t u32Temp;
        ...
        u32Temp = g_usbd_CtrlMaxPktSize;
        USBD_MemCopy((uint8_t *)USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR(EP0), pu8Buf, u32Temp);
...
void USBD_CtrlIn(void)
{
    ...
    if(g_usbd_CtrlInSize)
    {
      ...
      u32Temp = g_usbd_CtrlMaxPktSize;
        if(g_usbd_CtrlInSize > u32Temp)
        {
			// Data size > MXPLD
			u32Temp = g_usbd_CtrlMaxPktSize;
			USBD_MemCopy((uint8_t *)USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR(EP0), (uint8_t *)g_usbd_CtrlInPointer, u32Temp);
			USBD_SET_PAYLOAD_LEN(EP0, g_usbd_CtrlMaxPktSize);
			g_usbd_CtrlInPointer += g_usbd_CtrlMaxPktSize;

			u32Temp = g_usbd_CtrlMaxPktSize;
			g_usbd_CtrlInSize -= u32Temp;
        }
        else
        {
			// Data size <= MXPLD
			u32Temp = g_usbd_CtrlInSize;
			USBD_MemCopy((uint8_t *)USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR(EP0), (uint8_t *)g_usbd_CtrlInPointer, u32Temp);
    ...
    else // No more data for IN token
    {
        ...
        {
			u32Temp = USBD_GET_ADDR();
			if((u32Temp != g_usbd_UsbAddr) && (u32Temp == 0))
			{
		...
		if ( g_usbd_CtrlInSizeZeroFlag ) 
		{
			USBD_SET_PAYLOAD_LEN(EP0, 0);	
			g_usbd_CtrlInSizeZeroFlag = 0;
		}
...
{
    uint32_t u32Size, u32Temp;

    DBG_PRINTF("Ctrl Out Ack %d\n", g_usbd_CtrlOutSize);

    u32Temp = g_usbd_CtrlOutSizeLimit;
    if(g_usbd_CtrlOutSize < u32Temp)
    {
...
/*@}*/ /* end of group I94100_USBD_EXPORTED_FUNCTIONS */

/*@}*/ /* end of group I94100_USBD_Driver */
