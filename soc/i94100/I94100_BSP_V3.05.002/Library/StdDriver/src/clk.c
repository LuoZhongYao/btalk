...
{
	uint32_t u32PCLKDiv0, u32PCLKDiv1, u32HCLKFreq, u32PCLKDivMsk = 0;
	uint32_t u32MaxDiv = ((CLK_PCLKDIV_PCLK0DIV16&CLK_PCLKDIV_APB0DIV_Msk)>>CLK_PCLKDIV_APB0DIV_Pos);
	
	u32HCLKFreq = CLK_GetHCLKFreq();
	
	if( (u32PCLKDiv0=((u32PCLKDiv&CLK_PCLKDIV_APB0DIV_Msk)>>CLK_PCLKDIV_APB0DIV_Pos))>0 )
	{
		if( (u32PCLKDiv0>u32MaxDiv) || ((u32HCLKFreq>>u32PCLKDiv0)>FREQ_100MHZ) )
			return 0;
		u32PCLKDivMsk |= CLK_PCLKDIV_APB0DIV_Msk;
	}
	if( (u32PCLKDiv1=((u32PCLKDiv&CLK_PCLKDIV_APB1DIV_Msk)>>CLK_PCLKDIV_APB1DIV_Pos))>0 )
	{
		if( (u32PCLKDiv1>u32MaxDiv) || ((u32HCLKFreq>>u32PCLKDiv1)>FREQ_100MHZ) )
			return 0;		
		u32PCLKDivMsk |= CLK_PCLKDIV_APB1DIV_Msk;
	}
	
	CLK->PCLKDIV = ((CLK->PCLKDIV&(~u32PCLKDivMsk))|u32PCLKDiv);
...
/**
  ...
  * |\ref USBD_MODULE  	 |\ref CLK_CLKSEL4_USBSEL_HIRC					 |\ref CLK_CLKDIV0_USBD(x) |
  * |\ref USBD_MODULE  	 |\ref CLK_CLKSEL4_USBSEL_PLL 					 |\ref CLK_CLKDIV0_USBD(x) |
...
void CLK_SetModuleClock(uint32_t u32ModuleIdx, uint32_t u32ClkSrc, uint32_t u32ClkDiv)
{
    ...
    {
			// If the selected module is USBD, 
			if((MODULE_IP_EN_Pos(u32ModuleIdx) == MODULE_IP_EN_Pos(USBD_MODULE)) && (MODULE_APBCLK(u32ModuleIdx) == MODULE_APBCLK(USBD_MODULE)))
				/* Get clock select control register address */
        u32sel = (uint32_t)&CLK->CLKSEL4;
			else
        /* Get clock select control register address */
...
// Cal PLL clock variable(fvco)
#define CLK_CAL_FVCO(fcfg,no)           (fcfg*no)
// Cal PLL clock variable(fref)
#define CLK_CAL_FREF(fin,nr)            (fin/nr)

// Ref spec to check PLL clock's variable correctly(Configure frequency)
#define CLK_CHECK_FCFG(fcfg)            ((fcfg<=FREQ_200MHZ&&fcfg>=FREQ_50MHZ)?TRUE:FALSE)
// Ref spec to check PLL clock's variable correctly(fref)
#define CLK_CHECK_FREF(fref)            (((fref<=8000000UL)&&(fref>=4000000UL))?TRUE:FALSE)
// Ref spec to check PLL clock's variable correctly(fvco)
#define CLK_CHECK_FVCO(fvco)            ((fvco>=FREQ_200MHZ&&fvco<=FREQ_500MHZ)?TRUE:FALSE)
// Ref spec to check PLL clock's variable correctly(nr)
#define CLK_CHECK_NF(nf)                ((nf<=513&&nf>=2)?TRUE:FALSE)
...
{
	uint32_t u32ClkSrc,u32fin,u32no,u32nr,u32nf;
    ...
    else 
	{
        ...
        u32ClkSrc = CLK_PLLCTL_PLLSRC_HIRC;
		/* Input frequency */
		u32fin = __HIRC;
    }

	// Check PLL clock's variable correctly(Configure frequency)
    if(CLK_CHECK_FCFG(u32PllFreq))
	{
		// Decision no value.
		for( u32no=4; u32no>0; u32no>>=1 )
		{
			// Check PLL clock's variable correctly(fvco)
			if(CLK_CHECK_FVCO(CLK_CAL_FVCO(u32PllFreq,u32no)))
			{
				// Decision nr value.
				for( u32nr=32; u32nr>0; u32nr-- )
				{
					// Check PLL clock's variable correctly(fref)
					if(CLK_CHECK_FREF(CLK_CAL_FREF(u32fin,u32nr)))
					{
						// Cal nf value.(shift 2 bits to prevent overflow)
						u32nf = (((u32PllFreq>>2)>>1)*u32no*u32nr)/(u32fin>>2);
						// Check PLL clock's variable correctly(nr)
						if(CLK_CHECK_NF(u32nf))
						{
							/* Enable and apply new PLL setting. */
							CLK->PLLCTL = u32ClkSrc | ((u32no - 1UL) << CLK_PLLCTL_OUTDIV_Pos) | ((u32nr - 1UL) << CLK_PLLCTL_INDIV_Pos) | (u32nf - 2UL);
