 ...
 * @version  V1.00
 * $Revision: 6 $
 * $Date: 17/02/14 10:26a $
 * @brief    I94100 series Timer driver source file
 *
 * @note
...
uint32_t TIMER_Open(TMR_T *timer, uint32_t u32Mode, uint32_t u32Freq)
{
    ...
    return(TIMER_GetModuleClock(timer) / (u32Cmpr * (u32Prescale + 1)));
}
...
void TIMER_Delay(TMR_T *timer, uint32_t u32Usec)
{
    ...
    else
    {
        ...
        {
					// 100 micro second frequency = 10000Hz.
					u32Cmpr = (u32Usec * (u32Clk / 10000)) / 100;
        }
    }

    timer->CMP = u32Cmpr;
    timer->CTL = TMR_CTL_CNTEN_Msk | TIMER_ONESHOT_MODE | u32Prescale;
...
void TIMER_EnableCapture(TMR_T *timer, uint32_t u32CapMode, uint32_t u32Edge)
{
    ...
    timer->EXTCTL = (timer->EXTCTL & ~(TMR_EXTCTL_CAPFUNCS_Msk | TMR_EXTCTL_CAPEDGE_Msk)) |
                    u32CapMode | u32Edge | TMR_EXTCTL_CAPEN_Msk;
}
...
{
    timer->EXTCTL &= ~TMR_EXTCTL_CAPEN_Msk;
}
...
{
    timer->EXTCTL = (timer->EXTCTL & ~TMR_EXTCTL_CNTPHASE_Msk) | u32Edge;
    timer->CTL |= TMR_CTL_EXTCNTEN_Msk;
}
...
uint32_t TIMER_GetModuleClock(TMR_T *timer)
{
	...
	switch(u32Src)
	{
		...
		case 5: return __LIRC;
		case 7: return __HIRC;
	}
    return 0;
}
