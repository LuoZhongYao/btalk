...
/**
 * @brief       Enable Stride Mode
 *
 * @param[in]   u32Ch           The selected channel: 0~5
 *
 * @return      None
 *
 * @details     This function enable timeout function of the selected channel(s).
 * @note        This function is only supported 0~5 channels.
 *              This function must be operated in burst mode.
 */
void PDMA_EnableStride(uint32_t u32Ch)
{
	if(u32Ch > 5)
		return;
	
	// Stride mode must be operated in burst mode
	PDMA->DSCT[u32Ch].CTL &= ~(PDMA_DSCT_CTL_TXTYPE_Msk);

  PDMA_ENABLE_STRIDE(u32Ch);
}

/**
 * @brief       Set PDMA Stride Mode Configuration and Enable Stride Mode
 *
 * @param[in]   u32Ch           The selected channel: 0~5
 * @param[in]   u32SrcAddr   		Stride moe source address.
 * @param[in]   u32SrcOffset    Stride mode source address offset.
 * @param[in]   u32DescAddr     Stride moe destination address.
 * @param[in]   u32SrcOffset    Stride mode destination address offset.
 * @return      None
 *
 * @details     This function set the selected channel transfer mode. Include peripheral setting.
 */
uint8_t PDMA_SetStrideAddress(uint32_t u32Ch, uint32_t u32SrcAddr, uint32_t u32SrcOffset, uint32_t u32DstAddr, uint32_t u32DstOffset)
{
	if(u32Ch > 5)
		return 0;

	PDMA_SET_STRIDE_SRCOFF(u32Ch, u32SrcOffset);
	PDMA_SET_STRIDE_DESOFF(u32Ch, u32DstOffset);
	
	PDMA_SET_SRC_ADDR(u32Ch, u32SrcAddr);
	PDMA_SET_DST_ADDR(u32Ch, u32DstAddr);
	
	return 1;
}

/**
 * @brief       Set PDMA Stride Mode Transfer Count
 *
 * @param[in]   u32Ch           The selected channel: 0~5
 * @param[in]   u32StrideCount  Stride mode transfer count.
 * @param[in]   u32TransCount   PDMA channel transfer count
 * @param[in]   u32BurstSize    Set the size of burst mode. Valid values are
 *                - \ref PDMA_BURST_128
 *                - \ref PDMA_BURST_64
 *                - \ref PDMA_BURST_32
 ...
 * @details     This function set the selected channel transfer mode. Include peripheral setting.
 */
uint8_t PDMA_SetStrideCount(uint32_t u32Ch, uint32_t u32StrideCount, uint32_t u32TransCount, uint32_t u32BurstSize)
{	
	uint8_t u8Burst, u8i;
	...
	if(u32StrideCount >= 1 && u32StrideCount <= 0xFFFFUL)
		PDMA_SET_STRIDE_TRANS_CNT(u32Ch, u32StrideCount);
	else
		return 0;
		
	// Calculate the true burst size
	u32BurstSize = u32BurstSize >> PDMA_DSCT_CTL_BURSIZE_Pos;
	u8Burst = 1;
	for(u8i = 0; u8i < (0x07 - u32BurstSize); u8i++)
		u8Burst = u8Burst * 2;
	
	// Transfer count need to be less or equal burst size. And check register limit.
	if((u32TransCount <= u8Burst) && (u32TransCount-1 <= 0xFFFFUL) && (u32BurstSize <= 0x7UL))
	{
		PDMA_SET_TRANS_CNT(u32Ch, u32TransCount);
		PDMA->DSCT[u32Ch].CTL = (PDMA->DSCT[u32Ch].CTL & ~PDMA_DSCT_CTL_BURSIZE_Msk) | (u32BurstSize << PDMA_DSCT_CTL_BURSIZE_Pos);
	}
	else
		return 0;
