...
/**
 * @brief     	Enable PDMA stride functions
 * @param[in]  	u32Ch     The selected channel: 0~5
 * @return     	None
 */
#define PDMA_ENABLE_STRIDE(u32Ch)	((uint32_t)(PDMA->DSCT[(u32Ch)].CTL |= PDMA_DSCT_CTL_STRIDEEN_Msk))

/**
 * @brief      	Disable PDMA stride functions
 * @param[in]  	u32Ch     The selected channel: 0~5
 * @return     	None
 */
#define PDMA_DISABLE_STRIDE(u32Ch)	((uint32_t)(PDMA->DSCT[(u32Ch)].CTL &= ~(PDMA_DSCT_CTL_STRIDEEN_Msk)))

/**
 * @brief       Set Stride Destination Address Offset
 *
 * @param[in]   u32Ch     The selected channel: 0~5
 * @param[in]   u32Offset The address offset
 *
 * @return      None
 *
 * @details     This macro set the destination address stride transfer offset count of each row.
 */
#define PDMA_SET_STRIDE_DESOFF(u32Ch, u32Offset) (*(&(PDMA->ASOCR0) + (u32Ch)*2) = (*(&(PDMA->ASOCR0) + (u32Ch)*2) & ~(PDMA_ASOCR_DASOL_Msk)) | ((u32Offset) << PDMA_ASOCR_DASOL_Pos))

/**
 * @brief       Set Stride Source Address Offset
 *
 * @param[in]   u32Ch     The selected channel: 0~5
 * @param[in]   u32Offset The address offset
 *
 * @return      None
 *
 * @details     This macro set the Source address stride transfer offset count of each row.
 */
#define PDMA_SET_STRIDE_SRCOFF(u32Ch, u32Offset) (*(&(PDMA->ASOCR0) + (u32Ch)*2) = (*(&(PDMA->ASOCR0) + (u32Ch)*2) & ~(PDMA_ASOCR_SASOL_Msk)) | ((u32Offset) << PDMA_ASOCR_SASOL_Pos))
 ...
 * @details     This macro set the selected channel transfer count.
 */
#define PDMA_SET_STRIDE_TRANS_CNT(u32Ch, u32TransCount) (*(&(PDMA->STCR0) + (u32Ch)*2) = (*(&(PDMA->STCR0) + (u32Ch)*2) & ~(PDMA_STCR_STC_Msk)) | ((u32TransCount) << PDMA_STCR_STC_Pos))
