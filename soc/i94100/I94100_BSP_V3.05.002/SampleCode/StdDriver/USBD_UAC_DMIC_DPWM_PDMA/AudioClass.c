...
/*--------------------------------------------------------------------------*/
// Extern Functions
extern void DMIC_Init(uint32_t sr);
extern void DPWM_Init(uint32_t sr);
...
volatile uint32_t g_au32DMIC2PDMA_Buff[2][DMIC2PDMA_BUFF_LEN] = {0};
volatile uint8_t g_u8DMIC_Buff_Index = 1;
...
{	
	...
	if (g_u8AudioMicState == UAC_BUSY_AUDIO_RECORD)
	{
		...
		{   
			u16distance = (DMIC_RING_BUFFER_LEN_4CH - g_u16Record_Read_Ptr) + g_u16Record_Write_Ptr;	
		}
		...
		{           
			pu8buf[i*3+0] = (g_au32PDMA2USB_RingBuff[g_u16Record_Read_Ptr] >>  0) & 0xff;
			pu8buf[i*3+1] = (g_au32PDMA2USB_RingBuff[g_u16Record_Read_Ptr] >>  8) & 0xff;
			pu8buf[i*3+2] = (g_au32PDMA2USB_RingBuff[g_u16Record_Read_Ptr] >> 16) & 0xff;
			g_u16Record_Read_Ptr++;
			
			if (g_u16Record_Read_Ptr == DMIC_RING_BUFFER_LEN_4CH)       
			{
