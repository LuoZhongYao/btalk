  ...
  int dumpalloc, dumpfree;
{
	...
	if (bs < 0) {
	    bs = -bs;
            V BLOGD("Allocated buffer: size %6ld bytes.\n", (long) bs);
	    if (dumpalloc) {
		...
		(b->ql.flink->ql.blink != b)) {
            ...
            V BLOGD("Free block:       size %6ld bytes.%s\n",
		(long) bs, lerr);
#ifdef FreeWipe
  ...
  void *buf;
{
	...
	if (bs < 0) {
	    ...
	    if ((b->ql.blink->ql.flink != b) ||
		(b->ql.flink->ql.blink != b)) {
                V BLOGD("Free block: size %6ld bytes.  (Bad free list links)\n",
		     (long) bs);
		assert(0);
		return 0;
	    }
  ...
  char *when;
{
    ...
    V BLOGD("  %ld direct gets, %ld direct frees\n", ndget, ndrel);
#endif /* BECtl */
}
...
#ifdef COMPACTRACE
    V BLOGD("Compaction requested.  %ld bytes needed, sequence %d.\n",
	(long) bsize, seq);
#endif

    if (protect || (seq > CompactTries)) {
#ifdef COMPACTRACE
        V BLOGD("Compaction gave up.\n");
#endif
	return 0;
    }
...
void *malloc(unsigned size) __attribute__((alias("bget")));
void *realloc(void *ptr, unsigned newsize) __attribute__((alias("bgetr")));
void free(void *ptr) __attribute__((alias("brel")));
void *calloc(unsigned nmemb, unsigned size) __attribute__((alias("__calloc")));
void *__calloc(unsigned nmemb, unsigned size)
{
    return bgetz(size * nmemb);
}
